...
The `plot_similarity()` function will create plots summarizing repertoire overlap between samples. By default, Jaccard dissimilarity will be calculated and plotted as a heatmap. For this example, none of the samples show strong overlap. This may seem surprising since the MD4 samples are mainly composed of a single clonotype and should thus have a similar repertoire. However, Jaccard dissimilarity is only measuring the number of overlapping clonotypes and does not take into account clonotype abundance.
...
A circos plot can be created by setting the `method` argument to 'circos'. This plot will summarize the number of clonotypes overlapping between samples. The `group_col` argument can be used to split the graph into distinct sections.

The number of overlapping clonotypes is shown as the axis labels for each sample. The width of each link reflects the number of clonotypes overlapping the samples.
...
Multidimensional scaling (MDS) can be used to visualize the overall similarity between samples. The `calc_mds()` function will calculate MDS coordinates using either the Jaccard dissimilarity index, or the Horn-Morisita index. The method can be specified using the `method` argument. MDS coordinates will get added to the meta.data.
