...
# lorem ipsum
filler <- str_split(lorem, "")[[1]][1:500] %>%
  str_c(collapse = "")
```
...
paths        <- dir(here("data"), pattern = "[0-9]_AB")
names(paths) <- str_remove(paths, "_b_Cell$|_bCellEnriched$")
paths        <- map_chr(paths, ~ here("data", .x))
...
so_filt <- filter_vdj(
  sobj_in      = so_bcr,                                                 # Seurat object
  filt         = "IGH" %in% chains && any(c("IGL", "IGK") %in% chains),  # Condition for filtering
