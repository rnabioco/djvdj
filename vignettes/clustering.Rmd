...
Plot colors can be modified using the `plot_colors` argument and the number of rows used to arrange panels can be adjusted with the `facet_rows` argument. Like most djvdj plotting functions, `plot_motifs()` will return a ggplot2 object that can be modified with other ggplot2 functions.
