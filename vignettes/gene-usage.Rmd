...
The `plot_gene_usage()` function will summarize the frequency of each gene segment. By default if a single column is passed to the `gene_cols` argument, a bargraph will be returned. The number of top genes to include in the plot can be specified with the `n_genes` argument. Alternatively, a vector of gene names can be passed to the `vdj_genes` argument to plot a set of desired genes.
