...
so_avid <- calc_abundance(
  ...
  clonotype_col = "clonotype_id"            # Column containing clonotype IDs to use
)
```
...
plot_abundance(
  input        = so_avid,
  cluster_col  = "mouse",                   # Column containing cell clusters to compare
  n_clonotypes = 10,                        # Number of top clonotypes to plot
...
The function `plot_vdj_usage()` can be used visualize gene usage across clusters. Using the `yaxis` argument, the percentage of cells (percent) or total number of cells (frequency) expressing each gene can be shown. The number of top genes (most frequent) to plot can also be specified with `n_genes`.
...
When multiple gene columns are passed to `plot_vdj_usage()`, a list of plots will be returned, one for each cluster in the `cluster_col` column.
