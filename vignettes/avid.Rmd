...
For this vignette we are using AVID-seq data. This is a method developed by the Hesselberth lab which involves staining cells with DNA-tagged antigen. The DNA tag is similar to the tags present on CITE-seq antibodies and can be detected in a standard 10x 5' single-cell run. For this experiment we mixed splenocytes from BL6 and MD4 mice and then stained with a HEL-DNA conjugate. MD4 B cells are monoclonal and specifically bind HEL.   
...
Due to a technical issue, we do not detect a heavy chain for the MD4 cells. However, for other datasets `filter_vdj()` can be used to remove V(D)J data for cells that lack paired chains. Setting `filter_cells` to `FALSE` (the default) will keep all cells in the object, but the V(D)J data will be removed.
...
```{r "abund_plots", fig.width = 6, fig.height = 4}
plot_abundance(
  sobj_in      = so_avid,                   # Seurat object
  cluster_col  = "mouse",                   # meta.data column for grouping
