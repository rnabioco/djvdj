...
We can then overlay the new labels present in the `uniq_chains` column on a UMAP projection.
...
# Chains bar graphs
chain_bars <- so_avid@meta.data %>%
  ggplot(aes(type, fill = uniq_chains)) +
  geom_bar(position = "fill") +
  scale_fill_manual(values = chain_cols, na.value = "grey90") +
  djvdj_theme() +
  theme(
...
Clonotypes that do not have paired chains can be excluded when using the `import_vdj()` function. The `filter_vdj()` function can also be used to remove V(D)J data for cells that lack paired chains. Setting `filter_cells` to `FALSE` (the default) will keep all cells in the object, but the V(D)J information will be removed.

```{r}
 so_filt <- filter_vdj(
  input        = so_avid,                                                # Seurat object
  filt         = "IGH" %in% chains && any(c("IGK", "IGL") %in% chains),  # Condition for filtering
...
so_avid <- calc_abundance(
  input       = so_avid,                    # Seurat object
  cluster_col = "mouse"                     # meta.data column containing cell clusters to compare
)
```
...
plot_abundance(
  input        = so_avid,                   # Seurat object
  cluster_col  = "mouse",                   # meta.data column containing cell clusters to compare
  label_col    = "clonotype_id",            # meta.data column containing labels to use for plot
...
so_avid <- calc_similarity(
  input       = so_avid,                    # Seurat object
  cluster_col = "seurat_clusters",          # meta.data column containing cell clusters to compare
